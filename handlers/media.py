...
async def process_song_search(message: Message, query: str):
    ...
    try:
        ...
        if not valid_results:
            await status_msg.edit_text("❌ Bunday qo'shiq topilmadi. Boshqacha nomlab ko'ring.")
            return
...
@router.message(F.voice | F.audio | F.video | (F.document & F.document.mime_type.startswith(("audio/", "video/"))))
async def handle_shazam_media(message: Message):
    ...
    status_msg = await message.answer("🔍 Qo'shiq aniqlanmoqda...")
    ...
    try:
        ...
        if result:
            ...
            await message.answer("❌ Qo'shiq aniqlanmadi.")
    except Exception as e:
